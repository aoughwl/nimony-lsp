...
| 8 | Completion (member + Ctrl+Space), inlay hints, document symbols, and semantic tokens returned nothing on a real project | fix #7 relocated each file's compiled `.s.nif` into `nimcache/lsp/<file>/`, but every artifact READER still walked the top-level `nimcache/` (member completion ran a full compile then returned 0 fields) | route all readers through the same `moduleCacheDir(cfg, file)` the check writes to; cross-module walks (imported types, type hierarchy, workspace symbols) use an `allSNif` iterator over `nimcache/lsp/*/` | `0a9a885` — member completion returns the type's fields, Ctrl+Space returns 356 project symbols in 30ms, inlay 25 hints, all after fix #7 |
| 7 | **The deepest one**: on a real multi-file project the cache **never warmed** — every hover/definition/keystroke-check stayed a full ~1.3s recompile, forever ("Loading forever", "typing does nothing"). Warming externally helped for one generation, then it was cold again | nimony writes different artifacts for a module compiled as `--isMain` vs. as a dependency; the LSP checks every open file as its own main module into ONE shared nimcache, so opening a *second* file overwrites the first's artifacts and neither warms again (proven: registry.nim → 0.00s, one `check nodes.nim` → registry back to 1.25s, then nothing warms) | give each open file its own nimcache under `nimcache/lsp/<file>` (a file's own feature-checks still share it, keeping #1's within-file fix) | `54d7198` — interleaved hovers across files 71ms first / 10ms after, vs ~1.3s-each thrash before |
